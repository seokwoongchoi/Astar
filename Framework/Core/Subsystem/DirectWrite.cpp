...
void DirectWrite::Text(const wstring & text, const D3DXVECTOR2 & position, const wstring & fontName, const float & fontSize, const D3DXCOLOR & fontColor, const DWRITE_FONT_WEIGHT & fontWeight, const DWRITE_FONT_STYLE & fontStyle, const DWRITE_FONT_STRETCH & fontStretch)
{
	D2D1_RECT_F range;
	...
	auto brush = RegistBrush(fontColor);
...
IDWriteTextFormat * DirectWrite::RegistFormat(const wstring & fontName, const float & fontSize, const DWRITE_FONT_WEIGHT & fontWeight, const DWRITE_FONT_STYLE & fontStyle, const DWRITE_FONT_STRETCH & fontStretch)
{
	Font font = 
	{
		fontName,
	...
	if (!format)
	{
		...
		fonts.push_back(make_pair(font, format));
	}
